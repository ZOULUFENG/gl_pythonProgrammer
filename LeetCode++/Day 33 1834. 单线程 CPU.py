...
class Solution:
    def getOrder(self, tasks: List[List[int]]) -> List[int]:
        ...
        len_tasks = len(tasks)
        len_ans = 0
        while len_ans < len_tasks:

            for i in tasks:
                if i not in task_list and i not in end_list:
                    if i[0] <= time:
                        task_list.append(i)
            ...
            for i in task_list:
                if i[1] == mint:
                    for x, y in tasks:
                        if y == mint and x <= time:
                            tttt = tasks.index([x, y])
                            if tttt not in ans_list:
                                ans_list.append(tttt)
                                break
                    # ans_list.append(tasks.index(i[1]))
                    time += i[1]
                    t_tasks.remove(i)
                    task_list.remove(i)
